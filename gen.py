...
def writeTableEntry(f, n, fn, s, e, p):
    f.write(
        """
| {0} | {1} |  _{3}_ | link:{{attachmentsdir}}/presentations/{4}.pdf[{4}.pdf] 
""".format(n.value.title(), fn.value.title(), e.value.title().strip(), s.value.strip(), str(p.value).strip()))
